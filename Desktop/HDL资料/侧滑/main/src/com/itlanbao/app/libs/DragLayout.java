package com.itlanbao.app.libs;

import com.itlanbao.app.activity.HomeActivity;
import com.itlanbao.app.application.SoftApplication;
...
public class DragLayout extends FrameLayout {
	...
	SimpleOnGestureListener mGestureListener = new SimpleOnGestureListener() {
		public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
			if ((Math.abs(distanceX) > Math.abs(distanceY)) && distanceX < 0 && isDrag != false
				...
				return true;
			} else if ((Math.abs(distanceX) > Math.abs(distanceY)) && distanceX > 0 && isDrag != false
					&& mStatus == Status.Open) {
				return true;
			} else {
				return false;
